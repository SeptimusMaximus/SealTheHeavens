...
{
	public class FirstPrismBeam : ModProjectile
	{
		...
		private const float PiBeamDivisor = MathHelper.Pi / 6;
		...
		public override void AI()
		{
			...
			if (Projectile.type != ModContent.ProjectileType<FirstPrismBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<FirstPrismHoldout>())
			{
				Projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / 90f, 0f, 1f);
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * 6) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / 6) % BeamHueVariance + BeamColorHue;
		...
		private static Color GetInnerBeamColor() => Color.White;
